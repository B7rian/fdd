...
following features:
- Uses an easy to understand repository format - just
  files and folders with the same names as they had when
  they were backed up - that can be examined with 
  standard file managers, commandline tools, text
...
- Add to existing repositories, adding symlinks for
  duplicate files and checking integrity of the link
  targets as necessary  
- Supports restoration of files using existing filesystem
  navigation methods.  The repository is just another
  directory, so find the file you want and copy it to
  where you need it.
...
- Can use/add new storage devices without having to
  update old ones Can convert existing file-copy backups
  (made with rsync, cp, file managers) to fdd repo
