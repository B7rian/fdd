...
(** [clone_files] tries to copy 1 source file at a time
    to directory [dst] until it succeeds without a
...
let rec clone_files srcs dsts =
  match srcs with
  | [] -> Option.none
  | x :: tl -> (
      match FS.ue_to_opt (FS.clone_file x) dsts with
      | Option.None -> clone_files tl dsts
      | _ -> Option.some x)

(** [link_to_old_files] finds files that have been
...
let backup_new_files backup_dir src_map =
  ...
  Stringmap.iter_with_skips
    (fun _ (s, l) ->
      (let dst_copies, dst_links =
         N.backups l backup_dir (s + 1)
       in
       let* copied_file = clone_files l dst_copies in
       let link_target =
...
(** The algorithm for [backup_files] is something like
    ...
    Step 2 is mostly in [link_to_old_files] Step 3 is
    in [backup_new_files]
...
let backup_files src_map dst backup_dir =
  ...
  let _ =
    src_map
    |> link_to_old_files backup_dir dst_seq
...
let dump_checksums dir map =
  Out_channel.with_open_text (N.checksum_file dir)
    (fun oc -> Stringmap.fprintf oc "%s  %s\n" map)
  ...
  let backup_dir, src_hashmap =
    Unix.handle_unix_error
      (fun () ->
        ...
        ( dst |> N.next_backup_dir |> FS.mkdirs,
          src_file_list srcs dst
          |> Stringmap.of_results
               ~max_skips:(ncopies - 1) D.sha256sum ))
      ()
  in
  let file_map =
