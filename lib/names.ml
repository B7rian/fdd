...
module Make (FS : Filesystem.S) = struct
  ...
  (** [checksum_file d] is the path to the checksum
      file in backup directory [d] *)
  let checksum_file dir =
    Filename.concat dir "checksums"

  (** [next_copy_dir d] returns the next directory name
      for extra copies of files given the directory
      name for the current set of copies or an empty
      string if no extra cooy directories have been
      made yet. The given name [d] and the returned
      path do jot have the backup directory prepended
  *)
  ...
  (** [backups s d n] creates 2 lists of files to back
      up for source files [s] being backed up to [d]
      with the user having requested [n] copies of the
      file contents and all files in [s] having
      identical contents. The first list is a list of
      destinations for actual copies, and the second is
      a list of symlinks that can point at any of the
      files in the first list *)
