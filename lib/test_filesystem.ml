...
let%expect_test "find" =
  ...
  let _ = Unix.system "touch fdir1/fdir2/fdir3/file_c" in
  let s = Filesystem.find (fun _x -> true) ["fdir1"] in
  let t = Filesystem.find 
            (fun x -> x = "fdir1/file_a") ["fdir1"] in
  Seq.iter (Stdio.printf "[%s] ") s;
  Seq.iter (Stdio.printf "[%s] ") t;
  [%expect {| [fdir1/file_a] [fdir1/fdir2] [fdir1/fdir2/fdir3] [fdir1/fdir2/fdir3/file_c] [fdir1/fdir2/file_a] [fdir1/fdir2/file_b] [fdir1/file_a] |}];;
